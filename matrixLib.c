...
//Calculates and returns the covariance matrix of the specified matrix
float **covarianceMatrix(float **mat, int row, int col){

    //Gets the mean of each column (row means)
    float *means = rowMeans(mat,row,col);
    
    //Subtracts the mean from each element in each column
    for (int i = 0; i < row; i++){
        for (int j = 0; j < col; j++){
            mat[i][j] = mat[i][j] - means[j];
        }
    }

    //Gets the transpose of the matrix and performs matrix multiplication
    float **trans = matrixTranspose(mat,row,col);
    float **result = matrixMultiplication(trans,mat,col,row,row,col);

    //Divides each element in the mutltiplication result matrix by the number of rows
    for (int i = 0; i < row; i++){
        for (int j = 0; j < col; j++){
            result[i][j] = result[i][j] / row;
        }
    }
